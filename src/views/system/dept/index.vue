			...
			<el-table-column label="操作" align="center" class-name="small-padding fixed-width">
				<template #default="scope">
					<div v-if="scope.row.deptName !== '游客部门'">
						...
						<el-button link type="primary" icon="Connection" @click="openSelectUser(scope.row)" v-hasPermi="['system:dept:edit']">绑定</el-button>
						<el-button link type="primary" icon="Wallet" @click="rechargeDialog(scope.row)" v-hasPermi="['system:dept:edit']">充值</el-button>
					...
					<el-col :span="12" v-if="deptType != 4 && title !== '修改部门'">
						<el-form-item label="部门分类" prop="deptType">
							<el-select v-model="form.deptType" placeholder="请选择部门分类" clearable>
								<el-option v-for="dict in gf_dept_type" :key="dict.value" :label="dict.label" :value="dict.value" :disabled="Number(dict.value) <= Number(deptType)" />
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
